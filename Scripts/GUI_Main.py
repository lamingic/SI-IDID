...
class main_gui():
    ...
    def welcome(self):
        ...
        IMAGE_PATH = 'G:/pythonwork/exps/Scripts/idid_platformEG.jpg'
        ...
        button = bkrgframe.add(tk.Button(self.win_welcome, text="Start", width=20, font=buttonFont, bg='#cacFFc',
                                         command=self.create_tabs), int(WIDTH * 0.4), int(HEIGTH * 0.85))
    ...
    def create_tabs(self):
        ...
        self.winm.geometry("+330+30")
        self.winm.update()
        self.center_window(self.winm, 880, 1050)
        self.winm.wm_attributes('-topmost', 1)
        # menu bar -------------------------------------
        ...
        helpMenu.add_checkbutton(label="Quit(退出)", command=self.winm.destroy, variable=quitVar)
    ...
    def create_domain_frame(self, tab, id):
        ...
        self.message_CompareDomains['CB_' + frame_name] = ttk.Combobox(master, width=18,
                                                                       textvariable=self.message_CompareDomains[
                                                                           'chosen'])
    ...
    def create_model_frame(self, tab, id):
        ...
        self.message_CompareModels['CB_' + frame_name] = ttk.Combobox(master, width=18,
                                                                      textvariable=self.message_CompareModels['chosen'])
    ...
    def add_compare_models(self):
        ...
        for key in self.Solvers.keys():
            for keyi in self.Solvers.get(key).keys():
                ...
                if self.Solvers[key][keyi].get().__contains__('PM'):
                    self.popup_window_PM(self.ModelParameters.values.get(key).get(keyi))
                if self.Solvers[key][keyi].get().__contains__('ACO'):
                    self.popup_window_ACO(self.ModelParameters.values.get(key).get(keyi))
                if self.Solvers[key][keyi].get().__contains__('PSOC'):
                    self.popup_window_PSOC(self.ModelParameters.values.get(key).get(keyi))
                if self.Solvers[key][keyi].get().__contains__('GC'):
                    self.popup_window_GC(self.ModelParameters.values.get(key).get(keyi))
                if self.Solvers[key][keyi].get().__contains__('CM'):
                    self.popup_window_CM(self.ModelParameters.values.get(key).get(keyi))
    ...
    def popup_window_PM(self, solver):
        solver['parameters'] = nsp.PM_ps()
        PW.popup_window(solver['type'] + '@' + solver['pointer'], solver['parameters'])

    def popup_window_CM(self, solver):
        solver['parameters'] = nsp.CM_ps()
        PW.popup_window(solver['type'] + '@' + solver['pointer'], solver['parameters'])

    def popup_window_ACO(self, solver):
        solver['parameters'] = nsp.ACO_ps()
        PW.popup_window(solver['type'] + '@' + solver['pointer'], solver['parameters'])

    def popup_window_PSOC(self, solver):
        solver['parameters'] = nsp.PSOC_ps()
        PW.popup_window(solver['type'] + '@' + solver['pointer'], solver['parameters'])

    def popup_window_GC(self, solver):
        solver['parameters'] = nsp.GC_ps()
        PW.popup_window(solver['type'] + '@' + solver['pointer'], solver['parameters'])
    ...
    def create_simmulation_frame(self, tab, id):
        ...
        check3 = tk.Checkbutton(master, text="Analysis_all", width=9,
                                variable=self.widgets['Analysis_all_' + frame_name])
        check3.deselect()
    ...
    def exp(self):
        ...
        if len(self.CompareModels.keys()) > 0:
            ...
            self.print('>> Testing')
            self.fitness_method = self.ModelParameters.values['DID']['extending policy trees']['parameters'].values[
                'fitness_method']
            for keyd in self.CompareDomains.keys():
                ...
                for i in range(0, len(self.Simmodels.base)):
                    ...
                    self.Simmodels.IDID[keyd][keym] = IDID(self.CompareDomains.get(keyd), self.CompareModels.get(keym),
                                                           self.scr_message)  # parameters

                    # 创建一个IDID，方便后续使用
                    self.public_idid = IDID(self.CompareDomains.get(keyd), self.CompareModels.get(keym),
                                            self.scr_message)
                    ...
                    # gen i's policy tree
                    self.Simmodels.IDID[keyd][keym].gen_pathes(self.public_idid)

                    # fit_dict[keym] = self.Simmodels.IDID[keyd][keym].did.dbn.result['Fitness']
                    self.print('>>>> finish model @ domain: ' + keyd + ' @ model: ' + keym)
                # self.plot_fitness(fit_dict)
    ...
    def plot_fitness(self, fit_dict):
        fig = plt.figure(dpi=100, figsize=(8, 6))
        index = list()
        color_ = ['black', 'r', 'b', 'green', 'c']
        i = 0

        for key in fit_dict.keys():
            if key:
                x = np.arange(len(fit_dict.get(key)))
                value = fit_dict.get(key)
                value = [round(n, 2) for n in value]
                # if self.fitness_method == 'diversity':
                #     value = [round(n, 2) for n in value]
                # if self.fitness_method == 'reward':
                #     value = self.up_data(value)
                # data = self.up_data(v)
                # value = [round(n, 2) for n in data]
                # value = self.up_data(v)
                plt.plot(x, value, color=color_[i], linewidth=2)

                index.append(key)
            i += 1
        for i in range(len(index)):
            index[i] = index[i].split('--')[1].split('-')[1]
        method = []
        for keym in self.CompareModels.keys():
            method.append(keym)
        for i in range(len(method)):
            method[i] = method[i].split('--')[1]
            if method[i] != 'Exact':
                method[i] = method[i].split('-')[1]
        title = ''
        for i in range(len(method)):
            if method[i] == 'Exact':
                continue
            if i != (len(method) - 1):
                title = title + method[i] + ' and '
            else:
                title = title + method[i]
        t = copy.deepcopy(title)
        title = 'The fitness of ' + title
        # plt.title(title)
        plt.xlabel('generation')
        plt.ylabel(self.fitness_method)
        plt.legend(index, loc='best')
        plt.show()
        domain = [i for i in self.CompareDomains.keys()][0].split('-')[1]
        name = domain + '-' + t + '-' + datetime.datetime.now().strftime('%Y%m%d-%H%M%S')
        t_horizon = self.CompareDomains[[i for i in self.CompareDomains.keys()][0]].values['horizon_size']
        fig.savefig(self.pnames.Save_filepath + name + '-' + self.fitness_method + '-' + str(t_horizon) + '.png')

    def plot_reward(self, reward_dict):
        fig = plt.figure(dpi=100, figsize=(8, 6))
        xingzhuang = ['o-', '^-', 'D-', 's-', 'h-']
        color_ = ['black', 'r', 'b', 'green', 'c']
        color_error = ['black', 'r', 'b', 'green', 'c']
        count = 0
        indexs = list()
        c_time = datetime.datetime.now().strftime('%Y%m%d-%H%M%S')
        for meth in reward_dict.keys():
            m2 = meth.split('--')[1]
            if m2 == 'Exact':
                m = 'Exact'
            else:
                m = m2.split('-')[1] + ''
            indexs.append(m)
            reward_mean = reward_dict[meth]['mean']
            reward_std = reward_dict[meth]['std']
            values = reward_mean[0, :]
            SD = reward_std[0, :]
            num_mod_did = len(values)
            index = np.arange(num_mod_did) + 1
            plt.errorbar(index, values, yerr=SD, fmt=xingzhuang[count], ecolor=color_error[count],
                         color=color_[count], elinewidth=1, linewidth=0.6, capsize=3, alpha=0.8)
            count = count + 1
        plt.legend(indexs, loc='best')
        plt.show()
        fig.savefig(self.pnames.Save_filepath + self.fitness_method + '_' + c_time + '.png')

    def analysis(self, frame_name):
        # print('start analysis')
        d = [i for i in self.CompareDomains.keys()][0]
        belief = copy.deepcopy(self.CompareDomains[d].beliefs['test'])
        fitness_dict = dict()
        reward_dict = dict()

        for keym in self.CompareModels.keys():
            if not keym.__contains__('Exact--Exact'):
                fitness_dict[keym] = dict()
            reward_dict[keym] = dict()
        self.num = self.domain_parameters.values['run_size']
        ...
        for i in range(self.num):
            ...
            self.s_i = sim_mode_i_set[0]
            self.s_j = sim_mode_j_set[0]

            for keyd in self.CompareDomains.keys():
                for keym in self.CompareModels.keys():
                    reward_dict[keym][i] = dict()
                    r_each_dict[keym] = dict()

                    # mean
                    r_i = copy.deepcopy(
                        self.Simmodels.IDID[keyd][keym].result.rewards[test_style][
                            sim_mode_i_set[0] + ' ' + 'vs' + ' ' + sim_mode_j_set[0]]['mean'])
                    reward_dict[keym][i]['mean'] = r_i
                    r_each_dict[keym]['mean'] = r_i

                    # std
                    std_i = copy.deepcopy(
                        self.Simmodels.IDID[keyd][keym].result.rewards[test_style][
                            sim_mode_i_set[0] + ' ' + 'vs' + ' ' + sim_mode_j_set[0]]['std'])
                    reward_dict[keym][i]['std'] = std_i
                    r_each_dict[keym]['std'] = std_i

                    if not keym.__contains__('Exact--Exact'):
                        # fitness
                        f_i = copy.deepcopy(
                            self.Simmodels.IDID[keyd][keym].did.dbn.result['Fitness']['max'])
                        fitness_dict[keym][i] = f_i
                        f_each_dict[keym] = f_i

            for key in self.CompareDomains.keys():
                self.CompareDomains[key].update()
                self.CompareDomains[key].beliefs['test'] = belief

            if self.num > 1:
                self.exp()
            bar.update(1)
        # avg
        f_after, r_after = self.avg_data(fitness_dict, reward_dict)
        self.save_data_toExcel("avg", f_after, r_after)
        self.plot_fitness(f_after)
        self.plot_reward(r_after)
        bar.close()

    def save_data_toExcel(self, type, fitness, reward):
        c_time = datetime.datetime.now().strftime('%Y%m%d-%H%M%S')
        domain_name = self.CompareDomains[[i for i in self.CompareDomains.keys()][0]].values['domain_name']
        path = os.path.dirname(os.path.realpath(__file__)) + '\\Data\\'
        fitness_type = self.fitness_method
        t_horizon = self.CompareDomains[[i for i in self.CompareDomains.keys()][0]].values['horizon_size']
        name = domain_name + '_data_' + fitness_type + '_' + c_time + '_' + str(t_horizon) + '_' + type + '.xlsx'
        savepath = path + name

        wb = Workbook(savepath)
        w1 = wb.add_worksheet('fitness')
        w2 = wb.add_worksheet('reward')
        a = 0
        for fit in fitness.keys():
            w1.write(0, a, fit)
            for i in range(len(fitness[fit])):
                w1.write(i + 1, a, fitness[fit][i])
            a += 1
        b = 0
        c = 0
        for re in reward.keys():
            m2 = re.split('--')[1]
            if m2 == 'Exact':
                m = 'Exact'
            else:
                m = m2.split('-')[1] + ''
            w2.write(0, c, m)
            for key in reward[re].keys():
                w2.write(1, b, key)
                for i in range(len(reward[re][key][0])):
                    w2.write(i + 2, b, reward[re][key][0][i])
                b += 1
            c += 2
        wb.close()

    def save_data(self, fitness, reward):
        c_time = datetime.datetime.now().strftime('%Y%m%d-%H%M%S')
        domain_name = self.CompareDomains[[i for i in self.CompareDomains.keys()][0]].values['domain_name']
        path = os.path.dirname(os.path.realpath(__file__)) + '\\Data\\'
        fitness_type = self.fitness_method
        t_horizon = self.CompareDomains[[i for i in self.CompareDomains.keys()][0]].values['horizon_size']
        f_name = domain_name + '_fitness_dict_' + c_time + '_' + fitness_type + '_' + str(t_horizon) + '.txt'
        r_name = domain_name + '_reward_dict_' + c_time + '_' + fitness_type + '_' + str(t_horizon) + '.txt'
        with open(path + f_name, 'a+') as f:
            for key in fitness.keys():
                f.write(key + ':')
                np.savetxt(f, fitness[key], fmt='%f', newline=' ')
                f.writelines('\n')
        with open(path + r_name, 'a+') as f:
            for key in reward.keys():
                f.write(key + ':')
                f.writelines('\n')
                for kd in reward[key].keys():
                    f.write('   ' + kd + ':')
                    np.savetxt(f, reward[key][kd], fmt='%f', newline=' ')
                    f.writelines('\n')

    def avg_data(self, fitness_dict, reward_dict):
        f = dict()
        r = dict()
        for keym in self.CompareModels.keys():
            if not keym.__contains__('Exact--Exact'):
        ...
        for key in fitness_dict.keys():
            a = 0
            for i in fitness_dict[key].keys():
                a = a + np.array(fitness_dict[key][i])
            f[key] = list(a / len(fitness_dict[key].keys()))

        for key in reward_dict.keys():
            b = 0
            c = 0
            r[key] = dict()
            for i in reward_dict[key].keys():
                b = b + reward_dict[key][i]['mean']
                c = c + reward_dict[key][i]['std']

            r[key]['mean'] = b / len(reward_dict[key].keys())
            r[key]['std'] = c / len(reward_dict[key].keys())
        return f, r
    ...
    def write_csv_results(self, test_style, sim_mode_i_set, sim_mode_j_set):
        ...
        for key in domain_set:
            ...
            for i in range(0, len(domain_horizon_list.get(key))):
                ...
                for keyd in self.CompareDomains.keys():
                    if self.CompareDomains.get(keyd).values.get(
                            self.pnames.domain_parameters[0]) == key and self.CompareDomains.get(keyd).values.get(
                        self.pnames.domain_parameters[4]) == h:
    ...
    def create_dataset(self, domain_set, domain_horizon_list, columns_dict, rows_dict, filename, test_style=None,
                       sim_mode_i_set=None, sim_mode_j_set=None):
        dataset = dict()  #
    ...
    def write_csv_runtimes(self):
        ...
        for key in alg_set:
            ...
            for i in range(1, len(self.pnames.Steps.get('IDID').keys()) + 1):
                ...
                self.rows_dict.get(key).get('IDID')[self.pnames.Steps.get('IDID').get(i)] = str(
                    start + len_did_steps - 1 + i)
            start = start + len_alg_steps
        # columns headings
        domain_set = set()
        for keyd in self.CompareDomains.keys():
        ...
        for keyd in self.CompareDomains.keys():
            ...
            for keym in self.CompareModels.keys():
                ...
                for t in times:
                    rowt = row.get(t)
                    worksheet.write(column + rowt, times.get(t) * 24 * 3600)
        self.create_dataset_time(domain_set, domain_horizon_list, columns_dict, self.rows_dict,
                                 self.pnames.table_files[2])
        workbook.close()
    ...
    def on_offline_test(self, test_style, sim_mode_i_set, sim_mode_j_set):
        ...
        for keyd in self.CompareDomains.keys():
            ...
            b = self.get_beliefs(4, self.CompareDomains.get(keyd).DBNS.get('DID').num_ss)
            # self.Simmodels.DID[keyd].DomainParameters.beliefs['DID'] = b
            self.CompareDomains[keyd].beliefs['DID'] = b
            ...
            for keym in self.CompareModels.keys():
                ...
                if test_style == self.pnames.Test_style.get(2):
                    ...
                    if sim_mode_i_set.__contains__(self.pnames.Sim_mode[3]) or sim_mode_j_set.__contains__(
                            self.pnames.Sim_mode[3]):
    ...
    def get_beliefs(self, N, k):
        belief = dict()
        for i in range(0, N):
            belief[i] = self.normalize_belief(np.random.rand(1, k), copy=False)[0]
        return belief

    def normalize_belief(self, _d, copy=True):
    ...
    def test_sub(self, domain, idid, idid_test, sim_mode_i_set, sim_mode_j_set, test_style):
        ...
        # bar = tqdm(total=int(
        #     len(sim_mode_i_set) * len(sim_mode_j_set) * len(policy_dict_i.keys()) * len(policy_dict_j.keys())))
        for si in sim_mode_i_set:
            for sj in sim_mode_j_set:
                ...
                for modi in policy_dict_i.keys():
                    ...
                    for modj in policy_dict_j.keys():
                        ...
                        if si == self.pnames.Sim_mode[3] or sj == self.pnames.Sim_mode[3]:
                            reward = self.get_rewards_gui_tree(idid, domain, pathes_i, pathes_j, weights_i, weights_j,
                                                               si, sj, modi, modj)
                        else:
    ...
    def create_play_frame(self, tab, id):
        ...
        self.message_CompareDomains['CB_' + frame_name] = ttk.Combobox(master, width=18, textvariable=self.widgets[
            'domain_' + frame_name])
        self.message_CompareDomains['CB_' + frame_name]['values'] = list()
        self.message_CompareDomains['CB_' + frame_name].grid(column=1, row=self.rowid, padx=1, pady=2)
    ...
    def create_message_frame(self, tab, id):
        scrolW = 120
        scrolH = 20
        master = self.create_frame(tab, id, clspan=2)
        frame_name = self.pnames.GUI_Frames[id]
        self.scr_message = scrolledtext.ScrolledText(master, width=scrolW, height=scrolH, wrap=tk.WORD)
        self.scr_message.grid(column=0, row=self.rowid, sticky='WE', columnspan=2)
    ...
    def Tiger_plot(self, master):
        ...
        Agent_i = Agent(self.Tiger.get_canvas(), x=120, y=400, label_str='Agent i', id='Agent i', color_id=1,
                        status='left')
        Agent_j = Agent(self.Tiger.get_canvas(), x=380, y=400, label_str='Agent j', id='Agent j', color_id=2,
                        status='right')
    ...
    def create_plot_frame(self, tab, key_tab, id):
        ...
        coverage = ttk.Button(master, text="Coverage", width=10,
                              command=lambda: self.coverage(tab, frame_name, key_tab))
    ...
    def plot_data_sub(self, sim_mode_i_set, sim_mode_j_set, domain, test_style, tab, frame_name, key_tab):
        ...
        for si in sim_mode_i_set:
            for sj in sim_mode_j_set:
                ...
                win = self.popup_window_plot_fig(key_str, modi_keys, indexs, domain, key, test_style, bar_width,
                                                 num_mod_did_max, tab, frame_name, key_tab)
                wins.append(win)
        return wins

    def popup_window_plot_fig(self, key_str, modi_keys, indexs, domain, key, test_style, bar_width, num_mod_did_max,
                              tab, frame_name, key_tab):
        ...
        xingzhuang = ['o-', '^-', 'D-']
        color_ = ['r', 'black', 'b']
        color_error = ['darksalmon', 'grey', 'cornflowerblue']

        for keyd in self.CompareDomains.keys():
            for keym in self.CompareModels.keys():
                if not keym.__contains__('Exact--Exact'):
                    self.Simmodels.IDID[keyd][keym].did.dbn.result['Fitness']['max'] = self.f[keym]
                self.Simmodels.IDID[keyd][keym].result.rewards[test_style][
                    self.s_i + ' ' + 'vs' + ' ' + self.s_j] = self.r[keym]
        for modi in modi_keys:
            ...
            for keym in indexs:
                ...
                count = count + 1
            title = 'Reward @ ' + domain + ' @ ' + 'i\'s mod ' + str(modi) + '-' + key_str
            curr_time = datetime.datetime.now().strftime('%Y%m%d-%H%M%S')
            domain = domain.split('-')[1]
            name_ = 'Reward @ ' + domain + '-' + key_str + '-' + curr_time
            # plt.title(title)
        ...
        canvas.draw()
        fig.tight_layout()
        fig.savefig(self.pnames.Save_filepath + name_ + '-' + self.fitness_method + '-' + str(self.t_horizon) + '.png')
            ...
            for keym_A in self.CompareModels.keys():
                ...
                for keym_B in self.CompareModels.keys():
                    ...
                    for modi_a in policy_dict_A.keys():
                        ...
                        for modi_b in policy_dict_A.keys():
    ...
    def create_table_frame(self, tab, key_tab, id):
        ...
        table = ttk.Button(master, text="Table", width=10,
                           command=lambda: self.popup_window_Table(tab, frame_name, key_tab))
        table.grid(column=i + 4, row=self.rowid, padx=1, pady=2, sticky='WE')

        close = ttk.Button(master, text="Close all", width=10,
                           command=lambda: self.close_all_tables(frame_name, key_tab))
    ...
    def create_diversity_frame(self, tab, sufix, key):
        ...
        self.message_CompareDomains['CB_' + frame_name + sufix] = ttk.Combobox(self.widgets['master_ps' + sufix],
                                                                               width=20,
                                                                               textvariable=self.widgets[
                                                                                   'domain_div' + sufix])
        ...
        self.message_CompareModels['CB_' + frame_name + sufix] = ttk.Combobox(self.widgets['master_ps' + sufix],
                                                                              width=20,
                                                                              textvariable=self.widgets[
                                                                                  'model_div' + sufix])
    ...
    def diversity_confirm(self, sufix, frame_name):
        ...
        master = self.widgets['master_inner_frame_' + 'pt' + sufix] if not sufix == '' else self.widgets[
            'master_div_' + 'pt' + sufix]
        self.create_subframe('pt', sufix, width, height, pt_dict, pt_horizon_dict, action_list, observation_list,
                             textlist, master)
        ...
        master = self.widgets['master_inner_frame_' + 'st' + sufix] if not sufix == '' else self.widgets[
            'master_div_' + 'st' + sufix]
        self.create_subframe('st', sufix, width, height, pt_dict, pt_horizon_dict, action_list, observation_list,
                             textlist, master)

        self.create_message_div_frame(self.widgets[frame_name + sufix], sufix, pt_keys, frame_name)
        action = ttk.Button(self.widgets['master_div_psm' + sufix], text="Calculate", width=20,
                            command=lambda: self.calculate_diversity(frame_name, sufix,
                                                                     self.widgets[frame_name + sufix]))
    ...
    def create_message_div_frame(self, diversity, sufix, pt_keys, frame_name):
        ...
        self.widgets['scr_message_div' + sufix] = scrolledtext.ScrolledText(self.widgets['master_div_' + 'dst' + sufix],
                                                                            width=scrolW,
    ...
    def create_frame_div(self, key, text, framestr, sufix, scroll, width=None, cl=None, clspan=None):
        ...
        self.widgets['master_div_' + framestr + sufix] = ttk.LabelFrame(self.widgets[key + '_tab' + sufix], text=text,
                                                                        height=self.widgets['height'],
                                                                        width=self.widgets['width'])
        self.widgets['master_div_' + framestr + sufix].grid(column=cl, row=self.rowid, padx=8, pady=4,
                                                            columnspan=clspan)
        if scroll:
            self.widgets['master_sf' + framestr + sufix] = ScrolledFrame(self.widgets['master_div_' + framestr + sufix],
                                                                         width=width, height=self.widgets['height'])
            ...
            self.widgets['master_sf' + framestr + sufix].bind_scroll_wheel(
                self.widgets['master_div_' + framestr + sufix])
            # Create a frame within the ScrolledFrame
            self.widgets['master_inner_frame_' + framestr + sufix] = self.widgets[
                'master_sf' + framestr + sufix].display_widget(Frame)
            # Add a bunch of widgets to fill some space

    def create_subframe(self, framestr, sufix, width, height, pt_dict, pt_horizon_dict, action_list, observation_list,
                        textlist, master, col=None):
        ...
        for ei in pt_dict.keys():
            ...
            self.plot_policy_tree(self.widgets['policytrees_' + framestr + sufix][ei], policy_tree, horizon,
    ...
    def calculate_diversity(self, frame_name, sufix, diversity):
        ...
        self.create_frame_div(frame_name, 'Set of unique Triangles @H_' + str(modi), 'stm', sufix, scroll=scroll,
                              width=600, cl=1, clspan=1)
        ...
        master = self.widgets['master_div_' + 'stm' + sufix] if sufix == '' else self.widgets[
            'master_inner_frame_' + 'stm' + sufix]
    ...
    def create_policytree_frame(self, tab, id, key_tab):
        ...
        self.widgets['CB_' + frame_name] = ttk.Combobox(master, width=18,
                                                        textvariable=self.widgets['idid_' + frame_name])
    ...
    def plot_policytree(self, frame_name, master, key_tab, tab):
        ...
        self.widgets['policytrees_' + frame_name][id].grid(column=0, row=self.widgets['rowid_' + key_tab], padx=1,
                                                           pady=2)

...
class IDID(Model):
    ...
    def gen_pathes(self, public_idid):
        ...
        self.next_step(start=True)  # extending network
        self.dbn.extend(self.DomainParameters.values['horizon_size'], self.step)

        self.next_step(start=True, end=True)  # generating network
        self.dbn.generate_evidence(self.step)

        self.next_step(start=True, end=True)  # solving DID
        self.did.gen_pathes(public_idid)  # 处理DID的部分

        self.next_step(start=True, end=True)
        self.dbn.expa_policy_tree = self.did.dbn.result.get('policy_tree') # 将did的策略树赋值给IDID
        self.dbn.expansion(self.step)
        ...
        self.dbn.result.get('policy_tree').set_name(
            self.DomainParameters.Tostring() + '-' + self.ModelParameters.Name + '-' + str(self.step) +
